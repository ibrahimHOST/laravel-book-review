...
@section('content')
    ...
    <form method="GET" action="{{ route('books.index') }}" class="m-4 flex items-center space-x-2">
        <input class="input h-10" type="text" name="title" placeholder="Search By title" value="{{ request('title') }}">
        <input type=hidden name="filter" value="{{ request('filter') }}">
        <button class="btn h-10" type="submit" name="search">Search</button>
    ...
    <div class="filter-container mb-4 flex">
        @php
            $filters = [
                ...
                'highest_rated_last_6months' => 'Popular Last 6 Months'
            ];
        @endphp
